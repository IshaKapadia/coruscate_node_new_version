...
beforeAll(async function (){
  try {
    ...
    const company = dbInstance.collection('companies');
    insertedCompany = await company.insertOne({
      company_name: 'alarm',
      comapny_id: 104,
      id: '6261176f9ead232ffb75b736'
    });
  }
  catch (error) {
...
describe('POST /register -> if email and username is given', () => {
  test('should register a user', async () => {
    let registeredUser = await request(app)
      ...
      .send({
        'user_name':'copying',
        'password':'eeS9q4CTHNMFiRO',
        'email':'Okey.Schmidt@yahoo.com',
        'user_company':insertedCompany.insertedId,
        'designation':'driver',
        'experties':'Forward',
        'mobile_no':'Cotton',
        'user_id':146,
        'user_image':'Developer',
        'mobileNo':'(113) 772-0393',
        'username':'Rhiannon.Kulas',
...
describe('POST /forgot-password -> if email passed from request body is valid and OTP sent successfully', () => {
  test('should return success message', async () => {
    let user = await request(app)
      .post('/device/auth/forgot-password')
      .send({ 'email':'Okey.Schmidt@yahoo.com', });
